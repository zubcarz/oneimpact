...
### Datos
- `src/data/zones.ts`: importa `seedZones`/`seedAdvances` de shared y expone helpers `getZone(slug)`, `advancesByZone(slug)`.
...
- Test RNTL: `Dots` refleja el indice activo; `ZoneRow` dispara `onPress` con el slug.
