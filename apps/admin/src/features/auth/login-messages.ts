 ...
 *
 * `loginSchema` (packages/shared) declares `email()` and `min(1)` with no custom
 * message, so zod would answer its English defaults ("Invalid email address",
 * "Too small: expected string to have >=1 characters") and they would end up on
 * screen. Duplicating the schema in the admin to translate it is forbidden by
 * the contract rule, so this goes in as a per-parse error map.
 *
