...
describe('loginIssueMessage', () => {
  ...
  // The point of the map: the form must never show the English defaults of zod
  // ("Invalid email address"), and it must not redefine loginSchema to avoid it.
  // Since the item 09 merge, `loginSchema` (packages/shared) carries its own
  // Spanish messages, and in zod 4 a schema-level message wins over the map
  // passed to `parse`. So these two fields are answered by `shared` and the map
  // stays as the guard for anything `shared` leaves untranslated.
  it('keeps the errors of loginSchema in Spanish when used as its error map', () => {
